...
# hadoop
RUN curl -sk https://www.eu.apache.org/dist/hadoop/common/hadoop-2.9.0/hadoop-2.9.0-src.tar.gz | tar -xz -C /tmp/

# build native libs
RUN cd /tmp/hadoop-2.9.0-src && mvn package -Pdist,native -DskipTests -Dtar

# tar to stdout
RUN rm -r /tmp/hadoop-2.9.0-src/hadoop-dist/target/hadoop-2.9.0/lib/native/examples
CMD tar -czv -C /tmp/hadoop-2.9.0-src/hadoop-dist/target/hadoop-2.9.0/lib/native/ .

# Create native libs tar with:
# docker run --rm dvoros/docker-hadoop-build:2.9.0 > hadoop-native-64-2.9.0.tgz
